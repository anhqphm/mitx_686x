...
def compute_cost_function(X, Y, theta, lambda_factor, temp_parameter):
    ...
    selected_prob = np.choose(Y, H)
    c = -(1 / n) * np.sum(np.log(selected_prob)) + lambda_factor / 2 * np.sum(np.square(theta))
